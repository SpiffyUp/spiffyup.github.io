...
spiffyGDPR = function( options ){
    ...
    jQuery(document).ready(function($){
        ...
        // Rebind to country change when Infusionsoft reload section
        if(typeof billing_hook == 'function')
            var billing_hook_gdprold = billing_hook;

        billing_hook = function () {

            // Daisy chain for compatibility
            if(typeof billing_hook_gdprold == 'function'){
                billing_hook_gdprold();
            }

            // Bind to country field changes
            jQuery('#country').on('change.gdpr', function (e) {
